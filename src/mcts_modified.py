...
def rollout(board, state):
    ...
    while board.is_ended(state) == False:
        ...
        # #Update state
        # state = 
        # currBox = board.owned_boxes(state)
        # print(currBox)

        currBox = board.owned_boxes(state)
        nextAction = None
        for action in board.legal_actions(state):
            #Prioritize corners if can choose them.
            if action[2] == 0 and action[3] == 0 or action[2] == 0 and action[3] == 2 or action[2] == 2 and action[3] == 0 or action[2] == 2 and action[3] == 2:
                nextAction = action
                
                break
        #Otherwise random
        if nextAction == None:
            nextAction = choice(board.legal_actions(state))
